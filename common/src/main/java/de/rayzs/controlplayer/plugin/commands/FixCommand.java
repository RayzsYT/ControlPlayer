...
public class FixCommand extends Command {
    ...
    public FixCommand(final ControlPlayerAPI api, final List<String> commands) {
        super(
                commands.get(0),
        ...
        this.api = api;
        setPermission("controlplayer.fix");
    }
    ...
    public boolean execute(final @NotNull CommandSender bukkitSender, final @NotNull String label, final @NotNull String[] args) {
        ...
        if (args.length == 0 || args[0].equalsIgnoreCase(sender.getName())) {
            ...
            final Session session = api.getSessionProvider().getSession(player);

            if (session != null) {
                ConfigData.Message.FIX_SELF_IN_SESSION.send(player);
            } else {
                fixPlayer(player);
                ConfigData.Message.FIX_SELF_SUCCESS.send(player);
            }
        ...
        if(session != null) {
            ConfigData.Message.FIX_OTHER_IN_SESSION.send(sender);
            return true;
        }

        fixPlayer(target);
        ConfigData.Message.FIX_OTHER_SUCCESS.send(sender, "%player%", target.getName());
        return true;
    }
    ...
    public @NotNull List<String> tabComplete(final @NotNull CommandSender sender, final @NotNull String alias, final @NotNull String[] args) throws IllegalArgumentException {
        ...
        final List<String> results = Bukkit.getOnlinePlayers().stream().filter(other ->
                api.getSessionProvider().getSession(other.getUniqueId()) == null
    ...
    private void fixPlayer(final CPPlayer player) {
